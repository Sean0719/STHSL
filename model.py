...
class spa_cnn_local(nn.Module):
    def __init__(self, input_dim, output_dim, ):
        ...
        self.spaConv1 = nn.Conv3d(input_dim, output_dim, kernel_size=[args.kernelSize, args.kernelSize, 4], stride=1, padding=[int((args.kernelSize-1)/2), int((args.kernelSize-1)/2), 0])
        self.spaConv2 = nn.Conv3d(input_dim, output_dim, kernel_size=[args.kernelSize, args.kernelSize, 4], stride=1, padding=[int((args.kernelSize-1)/2), int((args.kernelSize-1)/2), 0])
        self.spaConv3 = nn.Conv3d(input_dim, output_dim, kernel_size=[args.kernelSize, args.kernelSize, 4], stride=1, padding=[int((args.kernelSize-1)/2), int((args.kernelSize-1)/2), 0])
        self.spaConv4 = nn.Conv3d(input_dim, output_dim, kernel_size=[args.kernelSize, args.kernelSize, 4], stride=1, padding=[int((args.kernelSize-1)/2), int((args.kernelSize-1)/2), 0])
...
class tem_cnn_local(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.temConv1 = nn.Conv3d(input_dim, output_dim, stride=[1, 1, 1], kernel_size=[1, args.kernelSize, 4], padding=[0, int((args.kernelSize-1)/2), 0])
        self.temConv2 = nn.Conv3d(input_dim, output_dim, stride=[1, 1, 1], kernel_size=[1, args.kernelSize, 4], padding=[0, int((args.kernelSize-1)/2), 0])
        self.temConv3 = nn.Conv3d(input_dim, output_dim, stride=[1, 1, 1], kernel_size=[1, args.kernelSize, 4], padding=[0, int((args.kernelSize-1)/2), 0])
        self.temConv4 = nn.Conv3d(input_dim, output_dim, stride=[1, 1, 1], kernel_size=[1, args.kernelSize, 4], padding=[0, int((args.kernelSize-1)/2), 0])
        self.act_lr = nn.LeakyReLU()
        self.drop = nn.Dropout(0.2)
...
class Hypergraph(nn.Module):
    def __init__(self):
        ...
        self.adj = nn.Parameter(torch.Tensor(torch.randn([30, args.hyperNum, args.areaNum * 4])), requires_grad=True)
        self.Conv = nn.Conv3d(args.latdim, args.latdim, kernel_size=1)
        self.act1 = nn.LeakyReLU()
        self.drop = nn.Dropout(args.dropRate)

    def forward(self, embeds):
        adj = self.drop(self.adj)
        tpadj = adj.transpose(2, 1)
        embeds_cate = embeds.transpose(2, 3).contiguous().view(embeds.shape[0], args.latdim, 30, -1)
        ...
        retEmbeds = retEmbeds.view(embeds.shape[0], args.latdim, 30, args.areaNum, 4).transpose(2, 3)
        return retEmbeds
...
class STHSL(nn.Module):
    ...
    def forward(self, embeds_true, neg):
        ...
        spa_local2 = spa_local2.view(-1, 30, args.latdim, args.areaNum, 4).permute(0, 2, 3, 1, 4)
